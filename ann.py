...
class ANN():
	...
	def __init__(self,input_n,output_n,hidden_n,num_nodes,alpha,eta):
		...
		self.weight = [ [ [rand(random()) for k in range(sizes[i+1])] for j in range(sizes[i])] for i in range(num_layers-1)]
	...
	def feed_forward(self,layer):
		if layer == 0: #input layer and 1st hidden layer
			sum(map((lambda e: e[0]*e[1]),zip(self.weight[layer],self.input)))
